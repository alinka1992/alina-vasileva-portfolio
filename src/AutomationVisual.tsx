export default function AutomationVisual() {
  ...
  return (
    <svg className="data-visual automation-visual" viewBox="0 0 760 620" role="img" aria-label="Объёмный control center единого коммерческого контура">
      <defs>
      ...
      <g className="auto-back-panel">
        <polygon points="86,152 252,118 252,372 86,405" fill="url(#autoPanel)" />
        <polyline points="108,330 138,304 164,316 196,262 228,234" />
        {[0,1,2,3].map((i) => <rect key={i} x={112+i*30} y={182+i*18} width="18" height={72-i*8} rx="3" />)}
      </g>
      <g className="auto-back-panel">
        <polygon points="508,118 674,152 674,405 508,372" fill="url(#autoPanel)" />
        <circle cx="590" cy="242" r="42" /><circle cx="590" cy="242" r="27" /><path d="M590 200 A42 42 0 0 1 626 264" />
        {[0,1,2].map((i) => <rect key={i} x="536" y={318+i*18} width={94-i*20} height="7" rx="3" />)}
      </g>
      <g className="auto-center-panel">
        ...
        <text className="auto-center-kicker" x="380" y="136" textAnchor="middle">CONTROL CENTER</text>
        <text className="auto-center-title" x="380" y="176" textAnchor="middle">ЕДИНЫЙ КОНТУР</text>
        <g className="auto-mini-chart">
          <path className="auto-mini-area" d="M278 312 L278 284 C316 276 324 246 350 252 S390 226 414 214 S460 170 486 156 L486 312 Z" />
          <path className="auto-mini-line" d="M278 284 C316 276 324 246 350 252 S390 226 414 214 S460 170 486 156" />
          {[278,350,414,486].map((x,i) => <circle key={x} cx={x} cy={[284,252,214,156][i]} r="5" />)}
        </g>
